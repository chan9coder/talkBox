...
String action = "free_form.jsp";			//수정시 이동할 파일
...
try{
	...
	if(!rs.next()){ //rs에 데이터가 없으면(idx에 해당하는 게시글이 비회권이 아니면)
		out.print("<script>");
...
<form name="frm" action="<%=action%>" method="post">
	<input type="hidden" name="fl_ismem" value="n" /> <input type="hidden"
		name="idx" value="<%=idx %>" /> <input type="hidden" name="kind"
		value="<%=kind %>" /> <input type="hidden" name="cpage"
		value="<%=cpage %>" />
