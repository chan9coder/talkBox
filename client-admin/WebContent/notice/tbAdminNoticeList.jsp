...
try{
	...
	sql ="select nl_idx, nl_ctgr, nl_title, nl_read, if (curdate() = date(nl_date),"+ 
		" time(nl_date) ,replace(mid(nl_date, 3,8), '-', '.'))  nldate "
		+" from t_notice_list " + where + " order by nl_idx desc limit "+ start +", " + psize;
	rs = stmt.executeQuery(sql);
	

} catch(Exception e){
	out.print("공지사항 목록에서 문제가 발생했습니다.");
	e.printStackTrace();
}
%>
		...
		<td colspan="2" align="center">
			<form name="frmSch">
				<!-- action과 method속성은 각각 기본값인 현재 파일과 get을 사용할 것임 -->
					...
					<input type="button" value="전체 글 보기" onclick="location.href='/talkboxAdmin/notice/tbAdminNoticeList.jsp'">
			</form>
		</td>
	</tr>
</table><br />
<table width="700" border="0" cellpadding="0" cellspacing="0" id="list">
 ...
 if(rs.next()){
	 ...
	 do{
		 ...
		 title2 = "<a href ='tbAdminNoticeView.jsp?idx="+rs.getInt("nl_idx")+"&cpage="+cpage + schargs +"'";
...
<table width="700" border="0" cellpadding="5">
	<td width="*" align="right">
			<input type="button" class="btn2" value="등록" onclick="location.href='tbAdminNoticeForm.jsp?kind=in';" />
		</td>
</table>
...
<table width="700" border="0" cellpadding="5">
	...
	<tr>
		<td width="600" align="center">
			<%
 if (rcnt > 0){
	 String link = "tbAdminNoticeList.jsp?cpage=";
	 if (cpage == 1){
		 out.print("[처음]&nbsp;&nbsp;&nbsp;[이전]&nbsp;&nbsp;");
	 } else {
