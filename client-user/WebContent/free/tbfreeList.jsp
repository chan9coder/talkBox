...
String where = " where fl_isview = 'y' ";			//검색 조건이 있을 경우 where절을 저장할 변수
String fl_writer="", fl_ismem="n";
...
try{
	...
	// count() 함수를 사용하므로ResultSet 안의 데이터 유무를 검사할 필요는 없음
	
	pcnt = rcnt / psize;
	if(rcnt % psize > 0)	pcnt++;	//전체 페이지 수
	
	int start =(cpage -1) * psize;
	sql ="select fl_idx,fl_writer, fl_title, fl_read, fl_ismem, if (curdate() = date(fl_date),"+ 
		" time(fl_date) ,replace(mid(fl_date, 3,8), '-', '.'))  fldate "
	...
	//	if(fl_ismem.equals("y")) fl_writer=fl_writer.substring(0,5) + "***";
	//}

	
	//System.out.print("----"+fl_ismem);
		...
		<td colspan="2" align="center">
			<form name="frmSch">
				<!-- action과 method속성은 각각 기본값인 현재 파일과 get을 사용할 것임 -->
					...
					<input type="button" value="전체 글 보기" onclick="location.href='/talkbox/free/tbfreeList.jsp'">
			</form>
		</td>
	</tr>
</table><br />
<table width="700" border="0" cellpadding="0" cellspacing="0" id="list">
 ...
 if(rs.next()){
	 ...
	 do{
		 String title = rs.getString("fl_title");
		 fl_writer=rs.getString("fl_writer");
		 fl_ismem=rs.getString("fl_ismem");
		 if(fl_ismem.equals("y")) fl_writer=fl_writer.substring(0,5) + "***";
		 ...
		 title2 = "<a href ='tbfreeView.jsp?idx="+rs.getInt("fl_idx")+"&cpage="+cpage + schargs +"'";
...
%>
	...
	<%
	 ...
	 out.print("<tr height='30'><td colspan='5' align='center'>검색결과가 없습니다.</td></tr>"); 	
 }
 %>
</table>
<br />
...
<table width="700" border="0" cellpadding="5">
	<td width="*" align="right">
			<input type="button" class="btn2" value="등록" onclick="location.href='tbfreeForm.jsp?kind=in';" />
		</td>
</table>
<%} %>
<table width="700" border="0" cellpadding="5">
	...
	<tr>
		<td width="600" align="center">
			<%
 if (rcnt > 0){
	 String link = "tbfreeList.jsp?cpage=";
	 if (cpage == 1){
		 out.print("[처음]&nbsp;&nbsp;&nbsp;[이전]&nbsp;&nbsp;");
	 } else {
