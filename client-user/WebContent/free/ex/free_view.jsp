...
<table width="700" cellpadding="5">
	<tr>
		<th width="12%">작성자</th>
		<td width="13%"><%=(fl_ismem.equals("y")) ? fl_writer.substring(0,5) + "***" : fl_writer %></td>
		<th width="12%">작성일</th>
		<td width="*%"><%=fl_date %></td>
		<th width="12%">조회수</th>
...
if(fl_reply == 0){	//댓글이 없을 경우에만 수정과 삭제를 허용
	if(fl_ismem.equals("n")){	//현재 글이 비회원 글일 경우
		...
		if(isLogin && loginInfo.getMi_id().equals(fl_writer)){
...
</script> <input type="button" value="글삭제" onclick="isDel();" />
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <%} %> <input type="button"
			value="글목록" onclick="location.href='free_list.jsp<%=args %>';" /></td>
	</tr>
</table>
...
<tr align="left" valign="top">
	...
	<% if (isLogin &&loginInfo.getMi_id().equals(rs.getString("mi_id"))) {%>
		<img src="../img/d.png" width="20" title="댓글 삭제" class="hand" onclick="replyDel(<%=rs.getInt("fr_idx") %>);" /><br />
		<img src="../img/u.png" width="20" title="댓글 수정" class="hand" onclick="replyUp(<%=rs.getInt("fr_idx") %>);" />
	<%} %>
	</td> 
</tr>
<tr><td colspan="5">
